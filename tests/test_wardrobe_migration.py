...
def test_asset_state_revision_upgrades_from_0006_and_backfills_existing_assets(monkeypatch):
    migration = _migration("0007_eve_wardrobe_asset.py")
    statements = []
    monkeypatch.setattr(migration.op, "execute", statements.append)

    migration.upgrade()

    assert migration.down_revision == "0006_eve_wardrobe_item"
...
def test_0006_remains_the_immutable_garment_only_schema(monkeypatch):
    migration = _migration("0006_eve_wardrobe_item.py")
    statements = []
    monkeypatch.setattr(migration.op, "execute", statements.append)
